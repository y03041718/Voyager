...
const Explore: React.FC = () => {
  ...
  const [showMockData, setShowMockData] = useState(false); // 改为false，默认显示词云图
  const [userLocation, setUserLocation] = useState<{ lat: number; lng: number } | null>(null);
  const [showWordCloud, setShowWordCloud] = useState(true); // 新增：控制词云图显示
  ...
  const handleModeChange = (mode: 'nearby' | 'region') => {
    setSearchMode(mode);
    if (mode === 'nearby') {
      setShowWordCloud(true); // 显示词云图
      setShowMockData(false);
      setRegionPois([]);
    } else {
      setShowWordCloud(false); // 隐藏词云图
      setShowMockData(false);
      // 如果已选择城市，加载POI
  ...
  // 新增：处理词云图点击
  const handleWordCloudClick = async (word: string) => {
    console.log('词云图点击:', word);

    // 判断是文本搜索还是省市搜索
    if (word.includes('-')) {
      // 省市搜索格式：福建省-厦门市
      const [province, city] = word.split('-');
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-black tracking-tight">
              {showWordCloud ? '热门搜索' : showMockData ? '为您推荐' : '搜索结果'}
            </h2>
          ...
          {/* 词云图展示区域 */}
          {showWordCloud && searchMode === 'nearby' && (
              <div className="bg-white rounded-3xl shadow-sm p-8 mb-8">
                <div className="mb-6">
                  <h3 className="text-lg font-black text-on-surface mb-2">热门搜索词云</h3>
                  <p className="text-sm text-on-surface-variant">点击词云中的关键词快速搜索</p>
                </div>
                <WordCloudChart type="text" onWordClick={handleWordCloudClick} />
              </div>
          )}

          {filteredDestinations.length === 0 && !loading && !showMockData && !showWordCloud && (
